...
const SCENES := [
	...
	# The Backwards Warren. NOTE THE FILENAME: the display name is "The Backwards Warren"
	# and the script is ContrarianDepths.gd — a filename search for "warren" returns
	# nothing. Found only by grepping the string. Do not conclude a scene is absent from a
	# filename miss.
	["backwards_warren", ["res://src/maps/dungeons/ContrarianDepths.tscn", "res://src/maps/dungeons/ContrarianDepths.gd"]],
