...
func test_every_sfx_entry_loads() -> void:
	...
	assert_eq(unloadable.size(), 0,
		"%d sfx cue(s) are manifest-registered and CANNOT LOAD. RUN `XDG_DATA_HOME=$PWD/tmp/xdg godot --headless --audio-driver Dummy --import --quit` FIRST — a cold .godot cache after a merge produces exactly this and is the likeliest cause. If it survives an import, then check the .import sidecar is committed and the path still matches the file: %s" % [unloadable.size(), unloadable])
