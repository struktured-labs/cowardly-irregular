...
func test_command_menu_has_solo_duel_override_on_spotlight_gate() -> void:
	...
	assert_string_contains(window, "solo_duel_override",
...
func test_debug_and_solo_duel_are_both_honored_before_early_return() -> void:
	...
	var idx: int = src.find("autobattle_locked")
	var window: String = src.substr(idx, 800)
	assert_string_contains(window, "not debug_override and not solo_duel_override",
