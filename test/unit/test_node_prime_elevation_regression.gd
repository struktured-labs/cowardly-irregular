...
## Elevation regression for Node Prime's data platform (2026-09-06, CrossCode pass extension).
## Pins height_data/map_data dims, tier count, stair legality, and save-point reachability —
## the DataPlatformPad teleport pad counts as an extra edge alongside _can_step so the flood
## fill doesn't falsely report the platform unreachable if the stair regresses.
...
func test_height_data_matches_map_data_dims() -> void:
	...
	for row in _v._height_grid:
		assert_eq((row as Array).size(), 24, "24 cols per height row")
...
func test_at_least_two_distinct_tiers() -> void:
	...
	for row in _v._height_grid:
		for h in row:
			seen[h] = true
	assert_gte(seen.size(), 2, "data platform tier1 and the floor tier0 must both appear")
...
func test_elevator_has_two_endpoints_on_different_tiers() -> void:
	var edges := _elevator_edges()
	assert_gt(edges.size(), 0, "control: DataPlatformPad must be found under buildings")
	for pair in edges:
...
func test_save_point_reachable_from_spawn() -> void:
	...
	var platform_cell := _cell(_v.spawn_points["data_platform"])
	assert_true(reachable.has(platform_cell), "the data platform itself must be reachable from spawn")
