...
## Measured 2026-09-18 by a per-script probe over all 2,071 test files: it left a FREED Combatant in `enemy_party`. A dangling instance is worse than a stale value: `is`
## on a freed object is a script error that aborts whichever later function touches it.
##
