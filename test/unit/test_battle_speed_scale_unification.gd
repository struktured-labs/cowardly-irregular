...
func test_settings_load_migrates_pre_v2_files() -> void:
	...
	var mig: int = src.find("settings.get(\"speed_scale_v2\"")
	assert_gt(mig, -1, "settings load must check the speed_scale_v2 marker")
	var window: String = src.substr(mig, 400)
	assert_true(window.contains("_battle_speed_index = 1"),
		"pre-v2 files must reset to the true default (index 1 = label 1x = engine 0.5)")


func test_saved_settings_carry_the_marker() -> void:
	var src: String = FileAccess.get_file_as_string("res://src/save/SaveSystem.gd")
	assert_true(src.contains("\"speed_scale_v2\": true"),
