...
## canonical boss display name (BattleManager._gloat_boss_display_name)
...
func test_address_does_not_react_uses_canonical_resolver() -> void:
	...
	# The exact log line must call _gloat_boss_display_name.
	assert_true(src.contains("BattleManager._gloat_boss_display_name(persona_id)"),
		"'does not react' log line must use BattleManager._gloat_boss_display_name(persona_id) — canonical title from BossDialogue")
...
func test_gloat_boss_display_name_exists_on_battle_manager() -> void:
	...
	assert_true(bm_src.contains("func _gloat_boss_display_name(persona_id: String)"),
		"BattleManager._gloat_boss_display_name must exist — BattleCommandMenu calls it")


func test_gloat_boss_display_name_falls_back_to_prettifier() -> void:
	...
	# prettifier fallback.
	var idx: int = bm_src.find("func _gloat_boss_display_name")
	...
	assert_true(body.contains("persona_id.replace(\"_\", \" \").capitalize()"),
		"_gloat_boss_display_name must keep prettifier fallback for unknown personas")
	assert_true(body.contains("BossDialogue"),
		"_gloat_boss_display_name must consult BossDialogue.get_display_name")
