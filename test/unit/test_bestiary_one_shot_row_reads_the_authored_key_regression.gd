...
# that reds on correct work is how suppression entries get written in the first place.
	assert_gte(PREMISE_BOSSES.size(), 2,
		"PREMISE_BOSSES holds %d, fewer than the 2 this guard defends — the named-member check below is going vacuous. ADDING an anchor is free; losing one is not." % PREMISE_BOSSES.size())
