...
## Elevation regression for Brasston's Work Deck (2026-09-06, CrossCode pass extension).
## Pins height_data/map_data dims, tier count, stair legality, and save-point reachability —
## the WorkDeckLift elevator counts as an extra edge alongside _can_step so the flood fill
## doesn't falsely report the deck unreachable if a stair regresses.
...
## Bidirectional endpoint pairs of every VillageElevator under `buildings` — an edge the plain _can_step flood can't see.
...
func test_height_data_matches_map_data_dims() -> void:
	assert_eq(_v._height_grid.size(), 22, "22 height rows")
...
func test_at_least_two_distinct_tiers() -> void:
	...
	for row in _v._height_grid:
		for h in row:
			seen[h] = true
	assert_gte(seen.size(), 2, "Work Deck (tier1) and the market (tier0) must both appear")


func test_every_stair_connects_tiers_exactly_one_apart() -> void:
	var checked := 0
	for cell in _v._stair_cells:
		...
		for d in [Vector2i(1, 0), Vector2i(-1, 0), Vector2i(0, 1), Vector2i(0, -1)]:
			...
			if nh < 0:
				continue
			assert_lte(absi(nh - own_h), 1, "stair %s neighbour %s must not jump more than one tier" % [cell, n])
			if absi(nh - own_h) == 1:
				bridges_a_step = true
		assert_true(bridges_a_step, "stair %s must actually bridge a one-tier step" % cell)
		checked += 1
	assert_gt(checked, 0, "control: at least one stair checked")


func test_elevator_has_two_endpoints_on_different_tiers() -> void:
	var edges := _elevator_edges()
	assert_gt(edges.size(), 0, "control: WorkDeckLift must be found under buildings")
	for pair in edges:
...
func test_save_point_reachable_from_spawn() -> void:
	...
	var deck_cell := _cell(_v.spawn_points["work_deck"])
	assert_true(reachable.has(deck_cell), "the Work Deck itself must be reachable from spawn")
