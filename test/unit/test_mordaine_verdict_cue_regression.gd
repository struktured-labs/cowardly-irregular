...
func test_the_verdict_cue_actually_loads() -> void:
	...
	assert_not_null(stream, "ledger_close.ogg did not load. TWO causes are indistinguishable from here: (a) the .import sidecar was never committed, or (b) YOUR .godot cache is cold because this asset arrived in a merge — run `XDG_DATA_HOME=$PWD/tmp/xdg godot --headless --audio-driver Dummy --import --quit` and re-run BEFORE reading this as a defect. (b) is far likelier and looks exactly like someone else's asset being broken.")
