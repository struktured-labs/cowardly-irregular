...
- Music masters are 96kbps mono (desktop ships these verbatim). **WEB SHIPS THE WHOLE LIBRARY, TRANSCODED — it is NOT excluded, and `export_presets.cfg` is NOT the lever.** This line said "W4-W6 tracks are WEB-EXCLUDED via export_presets exclude_filter (procedural fallback)" and that describes a design that was REPLACED: `tools/make_web_audio.sh` transcodes every master to mono at `$WEB_AUDIO_KBPS`, and `make_web_stage.sh` stages that tier AND strips the live music patterns out of the staged preset's `exclude_filter`. `deploy_web.sh` says it outright in the comment above its audio-tier block — *"the lever is HERE and in make_web_stage.sh, never export_presets.cfg"* — and the committed filter carries no audio patterns at all, so the old sentence pointed at a mechanism that is empty. The swap was measured, not projected: the full library at a low bitrate is SMALLER than the partial library was at 96k, so web gained music rather than losing it. ⚠️ The excluded case still exists and is the OPT-OUT: `WEB_STAGE=0` takes the direct export and does ship without the W4-W6 endings — `deploy_web.sh` defaults it to **1** (`${WEB_STAGE:-1}`), so a reader who checks only `export_presets.cfg` concludes the opposite of what the default publish does. A pck gate re-reads the packed file table to confirm the tier the run asked for actually landed. itch.io HTML5 embeds cap single files at 200 MB; pipeline hard-fails on pck ≥ 190 MB
