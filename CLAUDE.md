...
- **Exit codes: `0` pass · `1` test failures · `2` bad invocation · `3` NOTHING RAN · `4` a test asserted nothing · `124` WEDGED.** Codes 2 and 3 were added 2026-07-29 because GUT **exits 0 when it runs no tests at all**, which no exit code could distinguish from a real pass. Measured: a nonexistent name gave `exit 0 · Scripts 1 · Tests 5`-shaped success with zero `Tests` lines. It cost the fleet four vacuous verification runs and two wrong diagnoses in one evening — cowir-ai briefly measured a known-red branch as green, and cowir-sfx took three attempts to get a real run. The causes are open-ended (absent file · empty `-gdir` · **fresh unimported worktree**, where `res://` does not resolve while the file sits on disk · a parse error that drops the script), so the wrapper does **not** enumerate them: it asserts the OUTCOME — a real run always prints a Totals block, a vacuous one never does — and exits 3 if none appeared. That vacuity check lives in the wrapper rather than in `tools/gate.sh` because `gate.sh` runs the full suite only — a single-file run never goes through it.
  ...
  ⚠️ **DO NOT WRITE THE CHECK AS `[ $? -eq 124 ]` — A TIMEOUT HAS FOUR EXIT CODES AND THE FLAGS PICK WHICH ONE.** Measured on this box (uutils 0.2.2), 3/3 each, by four lanes independently: `timeout N` → **124** · `--signal=TERM N` → **124** · `--kill-after=K N` → **125** · `--signal=TERM --kill-after=K` with a child that **traps TERM** → **137** (128+9). I first published this as uutils-vs-GNU; it is not, it is **`--kill-after`**, on one implementation on one box. ⛔ **And `sleep` dies on TERM, so every convenient measurement samples the COOPERATIVE case — a wedge is by definition the process that does NOT answer TERM**, which is why `--kill-after` is in the recommended form at all and why the code it then yields is the kill signal rather than a timeout code. `125` is also GNU's code for *"timeout ITSELF failed"*, so a handler written from the man page reads a wedge as broken tooling. **So: decide on `elapsed >= budget`; the code only corroborates.** `run_tests.sh` does exactly that and exits a normalised `124`, which is the number callers should test — a bare `timeout` you wrap by hand may give any of the four. **The first version of that arm tested `124` alone, so the wedge fell through to the vacuity check and reported itself as `3` — NO TESTS RAN, a different defect entirely; a positive control is the only reason that was caught.**
