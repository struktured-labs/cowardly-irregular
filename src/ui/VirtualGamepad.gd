...
const DPAD_SIZE = 72.0
const DPAD_BUTTON_SIZE = 44.0
const OPACITY = 0.45
...
func _create_buttons() -> void:
	...
	# D-pad (bottom-left)
	_dpad_center = Vector2(90, vp.y - 100)
	_create_dpad_button("ui_up", _dpad_center + Vector2(0, -DPAD_BUTTON_SIZE))
	_create_dpad_button("ui_down", _dpad_center + Vector2(0, DPAD_BUTTON_SIZE))
	...
	_create_action_button("battle_advance", Vector2(vp.x - 60, 50), "R", Color(0.5, 0.5, 0.7))

	# Start/Select (bottom center)
	_create_action_button("ui_menu", Vector2(vp.x / 2 + 40, vp.y - 36), "START", Color(0.5, 0.5, 0.5), true)
	_create_action_button("battle_toggle_auto", Vector2(vp.x / 2 - 40, vp.y - 36), "SEL", Color(0.5, 0.5, 0.5), true)
...
func _create_action_button(action: String, pos: Vector2, label_text: String, color: Color, small: bool = false) -> void:
	var radius = BUTTON_RADIUS if not small else 22.0
...
func _draw_circle_texture(radius: float, color: Color, label_text: String) -> TextureRect:
	...
	var lbl = Label.new()
	lbl.text = label_text
	lbl.add_theme_font_size_override("font_size", 11 if label_text.length() <= 2 else 8)
...
func _draw_dpad() -> void:
	var s = int(DPAD_SIZE * 2.4)
	var img = Image.create(s, s, true, Image.FORMAT_RGBA8)
	img.fill(Color(0, 0, 0, 0))

	var cx = s / 2.0
	var arm_w = DPAD_BUTTON_SIZE * 0.7
	var arm_len = DPAD_SIZE * 0.9
	...
	# Arrow labels
	for dir_data in [["^", Vector2(0, -30)], ["v", Vector2(0, 30)], ["<", Vector2(-30, 0)], [">", Vector2(30, 0)]]:
		var lbl = Label.new()
		lbl.text = dir_data[0]
		lbl.add_theme_font_size_override("font_size", 14)
		lbl.add_theme_color_override("font_color", Color(1, 1, 1, 0.6))
		lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		lbl.position = _dpad_center + dir_data[1] - Vector2(8, 10)
		lbl.mouse_filter = Control.MOUSE_FILTER_IGNORE
		add_child(lbl)
