...
## hint_for_action("battle_defer") with no pad returns the Defer KEY, and that key genuinely pages:
## page_delta's action check reads is_action_pressed, which a KEY event satisfies, so
## the Defer/Advance keys -> -1/+1 alongside KEY_PAGEUP/KEY_PAGEDOWN. Four routes, all live.
##
