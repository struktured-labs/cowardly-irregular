...
func _apply_save_data(data: Dictionary) -> void:
	...
	"""
	# Apply map/location: pre-set MapSystem.current_map_id (tick 308) as the
	# canonical hand-off — GameLoop's Continue / quick_load / in-game Load
	# flows all read it, tear down the current scene, and rebuild via
	# _start_exploration. The legacy MapSystem.load_map() call that used to
	# follow built a SECOND scene (Mode 7 overworld TWICE on web Continue —
	# stacked overlays, frozen shader uniforms, 2026-07-11 probe) and cached
	# the zombie in loaded_maps; every consumer rebuilds through GameLoop,
	# so state restore must never build scenes here.
	if data.has("map"):
...
func load_settings() -> void:
	...
	if GameState:
		...
		if settings.has("reduce_flashes"):
			GameState.reduce_flashes = bool(settings["reduce_flashes"])
