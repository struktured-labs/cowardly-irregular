...
const _FRAGMENT_GATES := {
	...
	"world5_fragment_tempo": {"flag": "cutscene_flag_tempo_futuristic_defeated", "after": "world4_tempo_defeat"},
	"world5_fragment_warden": {"flag": "cutscene_flag_warden_futuristic_defeated", "after": "world4_warden_defeat"},
	"world6_fragment_arbiter": {"flag": "cutscene_flag_arbiter_abstract_defeated", "after": "world5_arbiter_defeat"},
	"world6_fragment_curator": {"flag": "cutscene_flag_curator_abstract_defeated", "after": "world5_curator_defeat"},
	"world6_fragment_tempo": {"flag": "cutscene_flag_tempo_abstract_defeated", "after": "world5_tempo_defeat"},
	"world6_fragment_warden": {"flag": "cutscene_flag_warden_abstract_defeated", "after": "world5_warden_defeat"},
}
...
## skip is true only when that _ready consumes meta_dungeon_skip_pending. Whispering Cave does not — a pending skip must not look like floor 6.
## The read stays here rather than in a shared helper: DragonCave._ready and WhisperingCave._ready are source-pinned, and they also write the key and consume the skip. This check must not.
const _DEPTH_GATED_CAVE_FLOORS := {
	"whispering_cave": {"key": "whispering_cave_floor", "max": 6, "boss": "cave_rat_king_defeated", "skip": false},
	"castle_harmonia": {"key": "castle_harmonia_floor", "max": 4, "boss": "world1_mordaine_defeated", "skip": true},
}
...
func _opening_floor_for_map(map_id: String) -> int:
	...
	if _depth_boss_defeated(str(spec["boss"])):
		return 1
	var max_floor := int(spec["max"])
	if bool(spec.get("skip", false)) and bool(GameState.game_constants.get("meta_dungeon_skip_pending", false)):
		return max_floor
...
## Same boss lookup the cave's _load_boss_state uses: game_constants["dungeon_flags"] first, then the pre-tick-154 flag on the party leader. A missing constants key must not look like an empty dict, or a cleared castle with only the legacy flag would still read its deep saved floor.
func _depth_boss_defeated(boss_flag: String) -> bool:
	if GameState == null:
		return false
	var flags: Variant = null
	if GameState.game_constants.has("dungeon_flags"):
		flags = GameState.game_constants["dungeon_flags"]
	elif GameState.player_party.size() > 0 and GameState.player_party[0].has("dungeon_flags"):
		flags = GameState.player_party[0]["dungeon_flags"]
	else:
		return false
	return flags is Dictionary and bool((flags as Dictionary).get(boss_flag, false))
