...
func _setup_audio_players() -> void:
	...
	## Master, NOT MusicNight->MusicDuck: skips the night filter (it IS the night), the dialogue duck and the kill duck. REVIEWED 2026-09-11 (cowir-sfx): ambient tracks the MUSIC slider live, so the bypass only ever narrows a gap — 16 dB under music at rest, 6 dB worst case (kill landing mid-dialogue), never dominant. No change; re-open if voice acting ships.
