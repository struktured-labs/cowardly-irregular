...
func _setup_player() -> void:
	...
	player.name = "Player"
	player.position = spawn_points.get("default", Vector2(288, 256))
...
func _setup_controller() -> void:
	...
	controller.current_area_id = "vertex_village"

	controller.set_area_config("vertex_village", true, 0.0, [])
...
func _setup_save_point() -> void:
	...
	save_pt.position = Vector2(9 * TILE_SIZE, 6 * TILE_SIZE)
...
func _on_save_requested() -> void:
	if SaveSystem and SaveSystem.has_method("quick_save"):
		...
		print("[SAVE] Quick save triggered from The Vertex save point")
