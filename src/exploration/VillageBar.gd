...
## Prefer bespoke artist-anchored PNG frames at
## `res://assets/sprites/npcs/dancer/frame_<n>.png`; fall through to
## the legacy `_draw_dancer` pixel-by-pixel composite if any frame is
## missing. Struktured playtest (msg 2779): "fix the DANCERS in the bar
## in the first village so they're actually generated by a real sprite
## or using gpt-image." Bespoke 32x32 frames replace the 24x32 procedural.
...
func _generate_dancer_sprites() -> void:
	...
	# Fallback: 4-frame procedural composite (legacy path).
