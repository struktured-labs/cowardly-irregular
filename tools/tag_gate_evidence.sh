...
decide() {
    ...
    # NAME WHAT IS DIRTY, and say when it is a GENERATED sidecar. A bare count sends the reader
    # hunting: 2026-09-16, v3.33.362-alpha reported "1 uncommitted change(s)" and the file was
    # `assets/audio/sfx/party_ko.ogg.import` — regenerated by the publish's own import prewarm
    # because it had never been committed. That cost a FULL 25-minute suite on a tag the fold had
    # already gated, on every publish, until someone committed the sidecar. Both cowir-main and
    # this lane had to go find it by hand from the count.
    #
    # The sidecar case is worth naming separately because its remedy is cheap and specific
    # (commit the generated file) while a source edit's is not. Classification is by path, not by
    # guessing which extensions godot imports: an extension list produced 43 false positives here
    # (every .mp4/.ogv in assets/cutscene_videos has no sidecar and never gets one). What is on
    # disk after an import is the measurement; what godot might import is a model.
    local dirty_list
    dirty_list="$(git status --porcelain)"
    dirty="$(printf '%s' "$dirty_list" | grep -c . || true)"
    if [ "${dirty:-0}" -ne 0 ]; then
        local named sidecars extra=""
        named="$(printf '%s\n' "$dirty_list" | head -5 | sed 's/^/          /')"
        sidecars="$(printf '%s\n' "$dirty_list" | grep -c '\.import$' || true)"
        [ "$dirty" -gt 5 ] && extra=" (+$((dirty - 5)) more)"
        local note=""
        if [ "${sidecars:-0}" -eq "$dirty" ]; then
            note=" — ALL of them are generated .import sidecars; committing them makes this tag gate-clean and saves a full suite on every publish"
        elif [ "${sidecars:-0}" -gt 0 ]; then
            local _verb="is"; [ "$sidecars" -gt 1 ] && _verb="are"
            note=" — ${sidecars} of them ${_verb} generated .import sidecars, which are safe to commit"
        fi
        run "worktree has ${dirty} uncommitted change(s)${note} — the export ships the WORKING tree, which the fold never gated${extra}:
${named}"
    fi
...
tagged_delta_from: ${F40}")
    ...
    # ── the dirty-tree verdict NAMES its files, and classifies generated sidecars ──────────
    # A bare count sends the reader hunting: .362 reported "1 uncommitted change(s)" and the file
    # was a regenerated .import sidecar, which cost a full suite per publish until it was
    # committed. These arms pin the naming AND the classification, in both directions — the
    # sidecar claim must not appear for a source edit.
    msg() { # name tag  needle  want(yes|no)
        local name="$1" tag="$2" needle="$3" want="$4" out got
        out="$(cd "$sandbox" && "$SELF" "$tag" 2>&1)"
        case "$out" in *"$needle"*) got=yes ;; *) got=no ;; esac
        if [ "$got" = "$want" ]; then
            pass=$((pass+1)); printf '  ok    %-34s %s %s\n' "$name" "$want" "$needle"
        else
            fail=$((fail+1)); printf '  FAIL  %-34s got %s want %s for %s\n' "$name" "$got" "$want" "$needle"
        fi
    }
    # HEAD has moved since $sha — the delta cases above committed. Anchor on the CURRENT tree,
    # or the sha check fires first and these arms never reach the dirty branch (measured: they
    # did not, and the failure text named the marker mismatch rather than the dirtiness).
    local sha_now; sha_now="$(cd "$sandbox" && git rev-parse HEAD)"
    local ondisk_now; ondisk_now="$(cd "$sandbox" && ls test/unit/test_*.gd 2>/dev/null | wc -l)"
    (cd "$sandbox" && git tag -a dirtytag -m "rel
gated: ${sha_now} scripts=${ondisk_now} tests=9 passing=9 failing=0")

    (cd "$sandbox" && touch test/unit/probe.ogg.import)
    check "untracked sidecar"               RUN  dirtytag
    msg   "  ...names the file"             dirtytag "probe.ogg.import"     yes
    msg   "  ...says ALL are sidecars"      dirtytag "ALL of them are generated" yes

    (cd "$sandbox" && rm -f test/unit/probe.ogg.import && echo "x" >> test/unit/test_1.gd)
    check "source edit"                     RUN  dirtytag
    msg   "  ...names the file"             dirtytag "test_1.gd"            yes
    msg   "  ...claims NO sidecar"          dirtytag "sidecar"              no

    (cd "$sandbox" && touch test/unit/probe.ogg.import)
    msg   "mixed: counts the sidecars"      dirtytag "1 of them is generated" yes

    # the cap, so a 200-file dirty tree does not print 200 lines
    (cd "$sandbox" && for i in 1 2 3 4 5 6; do touch "test/unit/p${i}.ogg.import"; done)
    msg   "more than five: says how many"   dirtytag "more)"                yes

    (cd "$sandbox" && rm -f test/unit/probe.ogg.import test/unit/p*.ogg.import \
        && git checkout -- test/unit/test_1.gd)
    check "cleaned up again"                SKIP dirtytag
